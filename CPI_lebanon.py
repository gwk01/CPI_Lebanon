...
data=pd.read_excel('s3://cpistorage/CPI raw rebased series 2021.xlsx', sheet_name='Sheet1',storage_options={"key": key_id,"secret": secret_id,})
...
with col3:
    #Month
    st.markdown(f'<p style="text-align:left;color:#0d0d0c;font-family:Arial Black;font-size:30px;border-radius:0%;">Lebanon</p>', unsafe_allow_html=True)
...
if submit:
    if (gen_cpi == '') or (food_cpi=='') or (alcohol_cpi=='')or (cloth_cpi=='')or (house_cpi=='')or (furnish_cpi=='')or (health_cpi=='')or (trans_cpi=='')or (comm_cpi=='')or (leis_cpi=='')or (edu_cpi=='')or (rest_cpi=='')or (mis_cpi==''):
        ...
        try:
            ...
            data.to_excel('s3://cpistorage/CPI raw rebased series 2021.xlsx', sheet_name='Sheet1',storage_options={"key": key_id,"secret": secret_id,})
        except:
            pass
	...
	#send email
        ...
        msg['From'] = fromaddr
        msg['To'] = ", ".join(toaddr)
        msg['Subject'] = "NEW Lebanon CPI Data"

        body = "Kindly, find attached the recently submitted CPI Data by Lebanon NSO."
        ...
        server.starttls()
        server.login(fromaddr, 'gegsljknyjaccktm') #Type Password
        text = msg.as_string()
